...
-- Extend admins table with new columns (using ALTER TABLE for existing tables)
-- Note: SQLite doesn't support ALTER TABLE ADD COLUMN IF NOT EXISTS, so we check if columns exist
...
-- Cache username and full_name for search and display
-- These are updated when admin interacts with bot
-- username column already exists in the base schema
-- full_name column already exists in the base schema
